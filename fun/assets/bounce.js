...
function mount(stage, { word = "", palette, seed = 0, mode = "snake" } = {}) {
  ...
  function makeLetter(ch, cx, cy, vx, vy) {
    ...
    if (chaos) {
      ...
      /* Snake, Towers: one step per letter. Towers always runs the
         gradient once; Snake folds a long text forth → back → forth so
         the colour keeps moving from tile to tile. */
      const a = steps > 1 ? hue / (steps - 1) : 0;
      if (snake && steps > FOLD_ABOVE) { const b = a * 3; t = b <= 1 ? b : b <= 2 ? 2 - b : b - 2; }
      else t = a;                                  // Towers: always one straight run, first tile to last
    }
    if (!blank) hue++;
