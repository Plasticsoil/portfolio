/* ══════════════════════════════════════════════════════════════
   Sphere work-card — a live wireframe sphere matching the /sphere
   app's look: GRID only, small scale, high NOISE (organic wobble),
   SPIN. Pink mesh on black, hover speeds it up + tilts toward the
   cursor. Centered SPHERE logo (drop-shadow lifts it off the mesh)
   + grain. Falls back to the static PNG poster if WebGL is missing.
════════════════════════════════════════════════════════════════ */
(function () {
  'use strict';
  ...
  // unit-sphere lat/lon line-segment endpoints (pairs) for LineSegments
  function buildGrid(LAT, LON, SEG) {
    var v = [];
    ...
    for (i = 1; i < LAT; i++) {                       // latitude circles
      ...
      for (j = 0; j < LON; j++) {
        a = P(2 * Math.PI * j / LON, phi);
        b = P(2 * Math.PI * (j + 1) / LON, phi);
    ...
    for (j = 0; j < LON; j++) {                       // longitude meridians
      ...
      for (i = 0; i < SEG; i++) {
        a = P(theta, Math.PI * i / SEG);
        b = P(theta, Math.PI * (i + 1) / SEG);
        v.push(a[0], a[1], a[2], b[0], b[1], b[2]);
      }
    }
  ...
  // cheap, smooth, organic 3D wobble (multi-frequency trig) → mimics NOISE
  function wob(x, y, z, t) {
    return (
      Math.sin(x * 1.6 + t) * Math.cos(y * 1.4 - t * 0.7) +
      Math.sin(z * 2.0 + t * 1.2) * Math.cos(x * 1.2 + t * 0.4) +
      Math.sin(y * 2.2 - t * 0.9) * Math.cos(z * 1.7 + t * 1.1)
    ) / 3;
  }
  ...
  function initOne(canvas) {
    ...
    } catch (e) { return; }                       // no WebGL → static poster stays
    ...
    camera.position.z = 7.0;                       // pulled back → small sphere w/ margin
    ...
    var units = buildGrid(22, 30, 44);             // grid only — no dots
    var geo = new THREE.BufferGeometry();
    geo.setAttribute('position', new THREE.BufferAttribute(units.slice(), 3));
    var mesh = new THREE.LineSegments(geo, new THREE.LineBasicMaterial({
      color: 0xff8ff4, transparent: true, opacity: 0.55           // pink
    }));
    spin.add(mesh);
    ...
    function deform(t) {
      for (var i = 0; i < units.length; i += 3) {
        var ux = units[i], uy = units[i + 1], uz = units[i + 2];
        var r = R * (1 + AMP * wob(ux * FREQ, uy * FREQ, uz * FREQ, t));
        pos[i] = ux * r; pos[i + 1] = uy * r; pos[i + 2] = uz * r;
      }
    ...
    function frame() {
      ...
      tilt.rotation.y += ((hover ? px * 0.5 : 0) - tilt.rotation.y) * 0.06;
  ...
  function initAll() {
    if (!window.THREE) { setTimeout(initAll, 120); return; }
